...
function(error, record, search , dialog, message, utilities) {
    ...
    function saveRecord(scriptContext) {
    	try{
	    	log.audit('Inicio Grabar Configuracion Articulo','Submit - Tipo : Configuracion Articulo');
	        ...
	        if(!utilities.isEmpty(searchResult) && searchResult.length>0){
	    		alert('Solo puede cargar una configuraci' + '\u00f3' + 'n de Articulos. Verifique.');
	    		return false;
	    	}
	    	return true;
    	...
    	log.error('Grabar Configuracion Articulo','Submit - Excepcion Grabando Configuracion Articulo - Excepcion : ' + excepcion.message);
    }
    log.audit('Fin Grabar Configuracion Articulo','Submit - Tipo : Configuracion Articulo');
    return true;
    }
